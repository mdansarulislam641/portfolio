  ...
  const [message, setMessage] = useState('');
    ...
    return (
      ...
      <input value={name} required
          onChange={(e) => setName(e.target.value)}  className='bg-transparent border-2 border-teal-400 text-white placeholder:text-white input mt-1 input-bordered w-full' type="text" name="user_name" placeholder='Your Name' />
      </div>
      <div className='my-2'>
      <label  className='text-white text-xl'>Email</label>
      <input  value={email} required
          onChange={(e) => setEmail(e.target.value)} className='bg-transparent border-2 border-teal-400 text-white placeholder:text-white input mt-1 input-bordered w-full' type="email" name="user_email" placeholder='Your Email'/>
      </div>
      <div>
      <label className='text-white text-xl'>Message</label>
      <textarea  value={message} required
          onChange={(e) => setMessage(e.target.value)} className='input bg-transparent border-2 border-teal-400 text-white placeholder:text-white input-bordered w-full h-32 py-5' name="message"  placeholder='Your Message'/>
      </div>
